...
for pfx in 20180117 20200110; do
	...
	bcftools annotate --no-version -Ou -a $REFDUP -c CHROM,FROM,TO,JK -h /dev/stdin -S $wdir/$wdir.pass $mocha/$wdir.unphased.GRCh38.bcf | \
	...
	$eagle --geneticMapFile $REFMAP --outPrefix $mocha/$wdir.chr$chr.GRCh38 --numThreads 2 --vcfRef $kgp_pfx${chr}$kgp_sfx.bcf --vcfTarget $mocha/$wdir.unphased.GRCh38.bcf --vcfOutFormat b --noImpMissing \
	      --outputUnphased --vcfExclude $mocha/$wdir.xcl.GRCh38.bcf --chrom $chr --pbwtIters 3 && bcftools index -f $mocha/$wdir.chr$chr.GRCh38.bcf
    done
    ...
    bcftools +mocha --no-version -Ob -o $mocha/$wdir.mocha.GRCh38.bcf --threads 1 --rules GRCh38 --variants ^$wdir/$$mocha/$wdir.xcl.GRCh38.bcf -m $mocha/$wdir.mocha.tsv -g $mocha/$wdir.stats.tsv -u $mocha/$wdir.ucsc.bed -p $REFCNP --LRR-weight 0.2 --order-LRR-GC 2 $mocha/$wdir.GRCh38.bcf && bcftools index $mocha/$wdir.mocha.GRCh38.bcf
