...
for pfx in 20180117 20200110; do
	...
	bcftools annotate --no-version -Ou -a $REFDUP -c CHROM,FROM,TO,JK -h /dev/stdin -S $wdir/$wdir.pass $wdir/$mocha/$wdir.unphased.GRCh38.bcf | \
	bcftools +fill-tags --no-version -Ou -t ^Y,MT,chrY,chrM -- -t NS,ExcHet | \
	bcftools +mochatools --no-version -Ou -- -x $wdir/$wdir.sex -G | \
	bcftools annotate --no-version -Ob -o $wdir/$mocha/$wdir.xcl.GRCh38.bcf \
		 -i 'FILTER!="." && FILTER!="PASS" || JK<.02 || NS<'$ns' || ExcHet<1e-6 || AC_Sex_Test>6' \
		 -x FILTER,^INFO/JK,^INFO/NS,^INFO/ExcHet,^INFO/AC_Sex_Test && \
	bcftools index -f $wdir/$mocha/$wdir.xcl.GRCh38.bcf
    kgp_pfx="$REFDIR/ALL.chrs_GRCh38.genotypes.20170504.bcfs/ALL.chr"
    kgp_sfx="_GRCh38.genotypes.20170504"
    for chr in {1..22} X; do
	$eagle --geneticMapFile $REFMAP --outPrefix $wdir/$mocha/$wdir.chr$chr.GRCh38 --numThreads 2 --vcfRef $kgp_pfx${chr}$kgp_sfx.bcf --vcfTarget $wdir/$mocha/$wdir.unphased.GRCh38.bcf --vcfOutFormat b --noImpMissing \
	      --outputUnphased --vcfExclude $wdir/$mocha/$wdir.xcl.GRCh38.bcf --chrom $chr --pbwtIters 3 && bcftools index -f $wdir/$mocha/$wdir.chr$chr.GRCh38.bcf
    done
    bcftools view --no-version -Ob -o $wdir/$mocha/$wdir.other.GRCh38.bcf -t ^$(seq -s, 1 22),X,$(seq -f chr%.0f -s, 1 22),chrX $wdir/$mocha/$wdir.unphased.GRCh38.bcf && \
	bcftools index $wdir/$mocha/$wdir.other.GRCh38.bcf
    bcftools concat --no-version -Ou $wdir/$mocha/$wdir.{chr{{1..22},X},other}.GRCh38.bcf | \
	bcftools +mochatools --no-version -Ob -o $wdir/$mocha/$wdir.GRCh38.bcf -- -f $REFFA && \
	bcftools index $wdir/$mocha/$wdir.GRCh38.bcf
    bcftools +mocha --no-version -Ob -o $wdir/$mocha/$wdir.mocha.GRCh38.bcf --threads 1 --rules GRCh38 --variants ^$wdir/$$mocha/$wdir.xcl.GRCh38.bcf -m $wdir/$mocha/$wdir.mocha.tsv -g $wdir/$mocha/$wdir.stats.tsv -u $wdir/$mocha/$wdir.ucsc.bed -p $REFCNP --LRR-weight 0.2 --order-LRR-GC 2 $wdir/$mocha/$wdir.GRCh38.bcf && bcftools index $wdir/$mocha/$wdir.mocha.GRCh38.bcf
    cat $wdir/$mocha/$wdir.mocha.tsv | awk -v pfx="$wdir" 'NR==1 {print $0"\tURL"} \
    ...
    ln -s $wdir/$mocha/$wdir.stats.tsv
    $SUMPR --stats $wdir/$mocha/$wdir.stats.tsv --calls $wdir/$mocha/$wdir.mocha.tsv --pdf $wdir/$wdir.summary.pdf
    $MOCHR --mocha --cytoband $REFCYTO --png /tmp/test.png --vcf $wdir/$mocha/$wdir.mocha.GRCh38.bcf --samples 8033684140 --regions chr1:145696087-248956422
    $MOCHR --mocha --cytoband $REFCYTO --png /tmp/test2.png --vcf $wdir/$mocha/$wdir.mocha.GRCh38.bcf --samples 8033684079 --regions chr15:19847685-101991189
    $MOCHR --mocha --cytoband $REFCYTO --png /tmp/test3.png --vcf $wdir/$mocha/$wdir.mocha.GRCh38.bcf --samples 8037702308 --regions chr12:0-56613214
done
